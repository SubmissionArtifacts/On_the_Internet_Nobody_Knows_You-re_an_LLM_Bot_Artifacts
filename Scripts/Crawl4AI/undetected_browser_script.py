...
# Common instruction (“prompt”) for ALL pages
INSTRUCTION = """
...
def generate_test_cookieId(web_agent:str, siteKey:str, test_index:str, date:str, time:str):
    domain = domain_from_url(siteKey)
    print(f"domain is : {domain}")
    return {
            "name": "test_cookieId", 
            "value": f"{test_index}_{web_agent}_{date}_{time}", 
            "domain": str(domain),
            "path":"/"
        }
...
async def main():
    web_agent = "Crawl4AI_Undetected_Browser"
    ...
    # Per-run configuration (official pattern)
    ...
    out_path = f"./results_normal_prompt/crawl4ai_gpt-4o-mini_results_undetected_browser_{date_str}.csv"
    ...
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        ...
        for siteIndex, site in enumerate(SITES):
            for i in range(NB_RUNS_PER_SITE):
                ...
                cookies = generate_test_cookieId(web_agent,site,test_index,date_str,time_str)
